...
//	config.device_Type = "EFRTK4";
	#ifdef BOOT_LOADER
	NVIC_SetVectorTable(NVIC_VectTab_FLASH,BOOT_APP_BASE);
	#endif
	delay_init();	   	//延时函数初始化
...
void KEY_task(void *pdata)//任务函数
{
	...
	while(1) 
	{
		...
		if(KEY_ST==KEY_LONG_DOWN)
		{
			...
			{
				Task_Flag.Flag_Bit.Pow_On=TRUE;
				POWER_ON;//开机
			...
			{
				LED_STATU_OFF;
				Task_Flag.Flag_Bit.Pow_Off=FALSE;
		...
		else if((!Task_Flag.Flag_Bit.Pow_Off)&&(KEY_ST==KEY_LONG_UP))
		{
			Task_Flag.Flag_Bit.Pow_Off=TRUE;
		}
  }
}
...
void USART2_task(void *pdata)
{
	...
	while(1)
	{
		...
		if(BLUETOOTH_MESSG.receve_ok)
		{
			...
			{
				UART_SendString(BLUETOOTH_USART,&BLUETOOTH_MESSG.messg_buff[0]);//将其他数据发回蓝牙
			}
			...
			else if((des_mult=strstr(BLUETOOTH_MESSG.messg_buff,"$NET"))!=NULL)//网络地址
			{
				...
				Fram_Date_Buff[Fram_Date_Buff[0]]=0x20;//给定最后一个空的标志符
			...
			else if((des_mult=strstr(BLUETOOTH_MESSG.messg_buff,"$RTK"))!=NULL)//固定基站用
			{
				UART_SendString(GGA_USART,des_mult);	//发给底板去处理
			}
